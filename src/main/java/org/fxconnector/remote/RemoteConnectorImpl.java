...
class RemoteConnectorImpl extends UnicastRemoteObject implements RemoteConnector, FXConnector
{
    ...
    private void addStages( final int appsID, final StageID[] ids, final RemoteApplication application )
    {
        ...
        for( int i = 0; i < ids.length; i++ )
        {
            ...
            impl.getStages().add( new StageController()
            {
                ...
                @Override
                public void setDetail( final DetailPaneType detailType, final int detailID,
                    final String value )
                {
                    try
                    {
    ...
    public boolean isSupported( final VirtualMachine aMachine ) throws IOException
    {
        final String aProperty = aMachine.getSystemProperties().getProperty( JAVA_VERSION_PROPERTY_KEY );
        if( aProperty == null || aProperty.isEmpty()
            || Integer.parseInt( aProperty.split( "\\." )[ 0 ] ) < 11 )
        {
            return false;
        }
        return true;
    ...
    private void loadAgent( final VirtualMachine machine, final File agentFile )
    {
        try
        {
            Runtime.getRuntime().addShutdownHook( new Thread( () -> {
                ...
                {
                    ExceptionLogger.submitException( aE );
                }
            } ) );
            final long start = System.currentTimeMillis();
            final int port = getValidPort();
            Logger.print(
                "Loading agent for:" + machine + " ID:" + machine.id() + " on port:" + port + " took:" + (
                    System.currentTimeMillis() - start ) + "ms using agent defined in " + agentFile
                    .getAbsolutePath() );
            vmInfo.put( port, machine.id() );
            machine.loadAgent( agentFile.getAbsolutePath(),
                Integer.toString( port ) + ":" + this.port + ":" + machine.id() + ":" + Logger.isEnabled() );
            machine.detach();
        }
        catch( final Exception e )
        {
    ...
    private static final String JAVA_VERSION_PROPERTY_KEY = "java.version";
    ...
    private List< VirtualMachine > getRunningJavaFXApplications()
    {
        ...
        for( int i = 0; i < machines.size(); i++ )
        {
            ...
            try
            {
                ...
                if( sysPropertiesMap != null && sysPropertiesMap.containsKey(
    ...
    private File findAgent()
    {
        ...
        if( tempf == null || !tempf.exists() )
        {
            ...
            final File buildLibsDir = new File( "target/" );
